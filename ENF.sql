...
(
   ...
   IRR                  double default null comment '内部收益率',
   static_payback_time  double default null comment '静态投资回收年',
   dynamic_payback_time double default null comment '动态投资回收期',
   LCOE                 double default null comment 'LCOE',
   npv                  double default null comment '净现值',
   power_asset_current_value double default null comment '电站资产累计现值',
   evaluation_content   varchar(600) default null comment '评价内容',
   document_review      varchar(600) default null comment '文件审查',
   project_quality_situation varchar(600) default null comment '工程建设质量和系统运行情况',
   project_invest_situation varchar(600) default null comment '项目建设投资情况',
   project_earnings_situation varchar(600) default null comment '项目经济收益情况',
   evaluation_result    varchar(600) default null comment '评价结果',
   doc_mul              varchar(100) default null comment '附件集合',
   duty_person          varchar(20) default null comment '责任人',
...
(
   ...
   project_area         varchar(10) default null comment '项目地区',
   project_address      varchar(60) default null comment '项目详细地址',
   picture_full         varchar(100) default null comment '地面全景',
   picture_field        varchar(100) default null comment '场平图片',
   picture_transformer  varchar(100) default null comment '变电站图片',
   picture_mul          varchar(100) default null comment '动态图片集合', 
   contract             varchar(20) default null comment '合同的docID',
   project_name   varchar(50) default null comment '项目名称',
   contacts_name   varchar(20) default null comment '联系人名称',
   contacts_phone   varchar(11) default null comment '联系方式',
   contacts_email   varchar(50) default null comment '邮件地址',
   project_finish_date datetime comment '项目完工时间',
   project_electricity_price double default null comment '项目电价',
   project_investment double default null comment '项目总投资',
   ground_property      int default 1 comment '土地性质（0其他、1一般农田、2林地、3荒地、4鱼塘、5基本农田）',
   ground_property_other varchar(20) default null comment '其土地性质信息',
   ground_area          double default null comment '租赁土地面积',
   rent_time            double default null comment '租赁年限',
   rent_pay             double default null comment '租赁租金',
   control_room_area    double default null comment '中控室建筑面积',
   sell_sum    double default null comment '出让金额',
   ...
   voltage_level     varchar(50) default null comment '并网电压等级',
   ...
   rent_time2           double default null comment '租赁年限',
   rent_pay2            double default null comment '租赁租金',
...
(
   ...
   project_area         varchar(10) default null comment '项目地区',
   project_address      varchar(60) default null comment '项目详细地址',
   picture_full         varchar(100) default null comment '屋顶全景',
   picture_south        varchar(100) default null comment '屋顶正南图片',
   picture_mul          varchar(100) default null comment '动态图片集合', 
   contract             varchar(20) default null comment '合同的docID',
   project_name   varchar(50) default null comment '项目名称',
   contacts_name   varchar(20) default null comment '联系人名称',
   contacts_phone   varchar(11) default null comment '联系方式',
   contacts_email   varchar(50) default null comment '邮件地址',
   housetop_owner       varchar(50) default null comment '屋顶业主名称',
   company_type         int comment '企业类型（1国企（上市公司）、2外企（上市公司）、3私企（上市公司）、4国企（非上市公司）、5外企（非上市公司）、6私企（非上市公司））',
   company_capital      double default null comment '注册资本金',
   housetop_property_prove varchar(20) default null comment '屋顶产权证明（附件ID）',
   electricity_total    double default null comment '年用电量',
   electricity_pay      double default null comment '电费',
   electricity_pay_list varchar(20) default null comment '最近一年电费明细（附件ID）',
   housetop_type        int default 1 comment '屋顶类型（0其他、1混凝土、2彩钢瓦）',
   housetop_type_other  varchar(20) default null comment '其他房屋类型信息',
   housetop_area        double default null comment '屋顶面积',
   electricity_data     double default null comment '历史发电量',
   housetop_age         double default null comment '屋顶使用寿命',
   housetop_direction   int default 1 comment '屋顶朝向（0其他，1正南，2东西向）',
   housetop_direction_other varchar(20) default null comment '其它屋顶朝向信息',
   housetop_waterproof_time double default null comment '屋顶防水周期',
   housetop_load        double default null comment '屋顶活载荷',
   has_shelter          varchar(8) default null comment '附近有无遮挡（1有、2无）',
   has_pollution        varchar(8) default null comment '有无污染源（1有、2无）', 
   transformer_capacity double default null comment '上级变压器容量',
...
(
   ...
   company_type         int default 1 comment '企业注册资本/类型：0其他、1国有企业、2中外合资、3外商独资、4集体企业、5民营企业',
   company_person       varchar(100) comment '企业法人',
   company_capital      double default null comment '企业注册资本',
